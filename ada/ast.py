...
class BaseTypeDecl(BasicDecl):
    ...
    def find_base_type_rebindings(target=T.BaseTypeDecl):
        """
        Given a target type node, browse the inheritance hierarchy of this type
        until the target is found, and return its associated rebindings.
...
class TypeDecl(BaseTypeDecl):
    ...
    def direct_primitive_subps():
        ...
        """
        scope = Var(Entity.declarative_scope)
        pkg = Var(scope._.parent.cast(BasePackageDecl).as_entity)
